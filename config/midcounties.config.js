...
function buildRequestId() {
    ...
    return `MidCountiesPerformanceTest-${todayAsCompact()}${suffix ? "-" + suffix : ""}`;
}

const REQUEST_ID_PREFIX = "MidCountiesPerformanceTest-";

export default {
    // === Identity ===
    name: "MidCounties",
    // Use THIS env var at runtime: -e TOKEN_CENTRALENGLAND="Bearer eyJ..."
    tokenEnv: "TOKEN_MIDCOUNTIES",

    // === Default single-endpoint fields (kept for backward compat; not used in ops loop) ===
    apiUrl: "https://uks-mcowm-webapi-dev-web.azurewebsites.net/api/v-20180601/markdown",
 ...
 operations: [
        {
            key: "markdown",
            method: "POST",
            url: "https://uks-mcowm-webapi-dev-web.azurewebsites.net/api/v-20180601/markdown",
